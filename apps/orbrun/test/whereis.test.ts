...
const SAVED =
  ...
  'name=caeo:race=Minotaur:cls=Fighter:char=MiFi:xl=3:sk=Armour:sklev=4:title=Covered:place=D::2:br=D:lvl=2:' +
...
describe('parseXlog', () => {
  it('reads a doubled colon as the colon it stands for, so a place arrives whole', () => {
    ...
    expect(parseXlog('name=caeo:place=D::2:status=saved')).toEqual({ name: 'caeo', place: 'D:2', status: 'saved' })
...
describe('parseWhereis', () => {
  it('reads the file the server publishes, as the home screen speaks it', () => {
    ...
    expect(describeCharacter(w.character)).toBe('caeo the Covered, Minotaur Fighter XL3')
...
describe('morgue directories', () => {
  it('takes the directory from the dump the server handed back', () => {
    expect(morgueDirOf(cdi, '/crawl/morgue/caeo/morgue-caeo-20260910-120000')).toBe('/crawl/morgue/caeo/')
    expect(morgueDirOf(cdi, 'https://crawl.dcss.io/crawl/morgue/caeo/caeo')).toBe('/crawl/morgue/caeo/')
    // never off the server the account belongs to
    expect(morgueDirOf(cdi, 'https://example.com/morgue/caeo/x')).toBeNull()
  })

  it('guesses the layouts dgamelaunch-config ships with, CDI’s first', () => {
    expect(morgueDirGuesses('caeo')).toEqual(['/crawl/morgue/caeo/', '/morgue/caeo/', '/rawdata/caeo/'])
  })
