...
const POSES: Pose[] = [
  ...
  { name: 'third-person', x: 11, y: 5, yaw: 0, pitch: deg(-5), view: 'third' },
  { name: 'third-cursor', x: 11, y: 5, yaw: deg(-45), pitch: deg(-5), view: 'third', cursor: { x: 9, y: 3, mode: 'target' } },
  { name: 'third-above-lid', x: 11, y: 5, yaw: deg(90), pitch: deg(-5), view: 'third', camHeight: 2.2 },
...
const r = new Render3d({ view: 'first', fov: 80, eyeHeight: 0.65, viewmodel: true, motion: false })
r.mount(canvas)
r.setTiles(tiles)
r.setScene(scene)
...
r.setMinibars({ hp: 9, hpMax: 20, mp: 3, mpMax: 5, showHp: true, showMp: true })
...
function apply(p: Pose) {
  const opts: Render3dOptions = { view: p.view ?? 'first', camHeight: p.camHeight ?? 0.75, viewmodel: p.hands ?? true }
