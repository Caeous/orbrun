...
export function controlsSheet(padKind: PadKind = 'generic'): HTMLElement {
  ...
  return h('div', { class: 'body' }, table,
    h('p', { class: 'hint' }, 'RB opens actions (potions, scrolls, spells, abilities); Select opens Travel, Equipment and Character tabs; Start opens the Orbrun menu with the game options. LB / RB or left / right switches Select tabs. Commands already on gamepad buttons are omitted. Up / down chooses a command; Page Up / Down pages. A / Start selects, B backs out. Browsing sends no game commands.'))
}
